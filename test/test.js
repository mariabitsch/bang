...
describe('bang CLI tool', () => {
  ...
  test('adds complex shebang with env -S for commands with arguments', () => {
    ...
    const content = fs.readFileSync(testFile, 'utf8');
    assert.strictEqual(content, '#!/usr/bin/env -S uv run --script\nprint("hello")');
  });
  ...
  test('processes multiple files from package.json config', () => {
    ...
    assert.strictEqual(content1, '#!/usr/bin/env node\nconsole.log("script1");');
    assert.strictEqual(content2, '#!/usr/bin/env -S python3 -u\nprint("script2")');
  });
  ...
  test('makes files executable after adding shebang', () => {
    ...
    if (process.platform !== 'win32') {
      assert.ok(!(initialStats.mode & parseInt('111', 8)), 'File should not be executable initially');
    }
    ...
    if (process.platform !== 'win32') {
      assert.ok(finalStats.mode & parseInt('100', 8), 'File should be executable by owner');
      assert.ok(finalStats.mode & parseInt('010', 8), 'File should be executable by group');
      assert.ok(finalStats.mode & parseInt('001', 8), 'File should be executable by other');
      assert.strictEqual(finalStats.mode & parseInt('111', 8), parseInt('111', 8), 'File should have full executable permissions');
    }
    
    // On Windows, we can't really test this the same way, so just verify the file exists
    assert.ok(fs.existsSync(testFile), 'File should still exist after processing');
  });
  ...
  test('handles various executable formats correctly', () => {
    const testCases = [
      ...
      { input: 'node --experimental-modules', expected: '#!/usr/bin/env -S node --experimental-modules' },
      { input: 'uv run --script', expected: '#!/usr/bin/env -S uv run --script' },
      { input: 'python3 -u -W ignore', expected: '#!/usr/bin/env -S python3 -u -W ignore' }
    ];
    ...
    testCases.forEach((testCase, index) => {
      ...
      const content = fs.readFileSync(testFile, 'utf8');
      assert.ok(content.startsWith(testCase.expected), 
        `Expected "${testCase.expected}" but got "${content.split('\\n')[0]}" for input "${testCase.input}"`);
    });
  });
});
